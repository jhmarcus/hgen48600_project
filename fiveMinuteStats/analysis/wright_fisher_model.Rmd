...
In other words the probability of transitioning from an allele count of $i$, at generation $t-1$, to an allele count of $j$, at generation $t$, is binomially distributed with size $2N$ and success probability being equal to the frequency of the $A$ allele at time $t-1$:
...
The horizontal facet is the starting frequency and the vertical facet is the effective population size. We see that as we increase the population size the less noisy the allele frequency trajectory as expected from our derivation. Additionally we see that probability of fixation or less is more likely when lower and higher starting allele frequencies (again as expected).
